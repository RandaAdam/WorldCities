...
export class CountryService
  ...
  getData(
    ...
    var url: string = this.getUrl('api/countries');
  ...
  get(id: number): Observable<Country> {
    var url: string = this.getUrl("api/countries/" + id);

    return this.http.get<Country>(url);
  }

  put(item: Country): Observable<Country> {
    var url: string = this.getUrl("api/countries/" + item.id);
  ...
  isDupeField(countryId: number,
    ...
    var url: string = this.getUrl("api/countries/IsDupeField");
